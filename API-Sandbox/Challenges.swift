...
internal func exerciseOne() {
    ...
    let lastName:String = userData["results"][0]["name"]["last"].stringValue
    let streetName:String = userData["results"][0]["location"]["street"].stringValue
    let cityName:String = userData["results"][0]["location"]["city"].stringValue
    let stateName:String = userData["results"][0]["location"]["state"].stringValue
    let postCode:String = userData["results"][0]["location"]["postcode"].stringValue
    let title:String = userData["results"][0]["name"]["title"].stringValue
    let email:String = userData["results"][0]["email"].stringValue
    let cellNumber:String = userData["results"][0]["cell"].stringValue
    
    print("\(firstName) \(lastName) lives at \(streetName) in \(cityName), \(stateName), \(postCode). If you want to contact \(title).\(lastName), you can email \(email) or call at \(cellNumber)")
...
internal func exerciseThree() {
     ...
     */
    var allMovies: [Movie] = []
    for json:JSON in allMoviesData{
        var newMovie:Movie = Movie(json: json)
